...
function stopScraperService(): Promise<void> {
  return new Promise(resolve => {
    scraperServer.closeAllConnections?.();
    scraperServer.close(() => resolve());
  });
}

function scraperFetch(urlParam: string, headers: Record<string, string> = {}): Promise<Response> {
...
describe('Scraper service', () => {
  it('1. valid URL + correct auth → 200 {html, url}', async () => {
    ...
    const resp = await scraperFetch(mockUrl(), { Authorization: 'Bearer test-secret' });
  ...
  it('3. valid URL, wrong secret → 401', async () => {
    const resp = await scraperFetch(mockUrl(), { Authorization: 'Bearer wrong-secret' });
    assert.equal(resp.status, 401);
  });
  ...
  it('5. non-http URL → 400', async () => {
    const resp = await scraperFetch('ftp://example.com/file', { Authorization: 'Bearer test-secret' });
    assert.equal(resp.status, 400);
  });
  ...
  it('7. upstream redirect → 200, url = final URL', async () => {
    ...
    const resp = await scraperFetch(redirectUrl, { Authorization: 'Bearer test-secret' });
    assert.equal(resp.status, 200);
    const body = await resp.json() as { html: string; url: string };
